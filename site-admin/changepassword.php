...
if(isset($_POST) && $_POST['SaveEdit'] == 'Update') {
	//$date = date('Y-m-d');
	//echo "hai"; exit;
		$sth = $db->query("SELECT * FROM login WHERE `guid`='".$_SESSION['rcm_logInUser']."'");
		$row = $sth->fetch();
		...
		$bth = $db->query("SELECT * FROM login WHERE `guid`='".$_SESSION['rcm_logInUser']."' AND password = '$oldpassword'");
		$count = $bth->rowCount();
		if($count > 0 ) {
	     $securitykey = uniqid();
    	...
    	$uth = $db->query ("UPDATE `login` SET `password`='$password',`securitykey`='$securitykey'  WHERE `guid`='".$_SESSION['rcm_logInUser']."'");
            if($uth > 0){?>
              <script>alert("Password updated successfully"); window.location="<?php echo URL ?>changepassword.php"; </script>
            <?php } else {
        	   $post_msg = '<h4 style="color: green;">Please try Again</h4>';
        	   header('location:'.URL.'changepassword.php?post_msg='.$post_msg);
            }
    	} else {
    	$post_msg = '<h4 style="color: red;">Invalid old Password</h4>';
    	header('location:'.URL.'changepassword.php?post_msg='.$post_msg);
    	}
}
...
<html lang="en">
  ...
  <body>
              ...
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" onSubmit="return valid()" name="form1">
                ...
                <div class="form-group">
                  <label>Confirm Password</label>
                    <input type="password" class="form-control" placeholder="Confirm Password" name="newpassword" required >
                </div>
	...
	function valid()
	{
	if(document.form1.password.value!=document.form1.newpassword.value)
	{
