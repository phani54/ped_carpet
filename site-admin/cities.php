...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `cities`(`cid`, `sid`, `name`) VALUES ('$cid', '$sid', '$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">City Successfully Added</h4>';
	header('location:'.URL.'cities.php?post_msg='.$post_msg);
	} else {
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `cities` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
...
<div id="wrapper">
  ...
  <div id="page-wrapper">
      ...
      <div class="col-lg-12">
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li class="active"><a href="<?php echo URL; ?>cities.php">Cities</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>cities.php?action=addNew">Add New </a></li>
                  <li><a href="<?php echo URL; ?>cities.php">View Cities</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title">
          <!-- PAGE TITLE ROW -->
          <h1>Admin <span class="sub-title"><a href="cities.php">Cities</a></span></h1>
        </div>
        <!-- /#ek-layout-button -->
	...
	?>
    <a href="<?php echo URL; ?>cities.php" class="btn btn-info btn-xs">Back </a>
    <div class="row">
      <div class="col-lg-8">
        <div class="portlet">
                ...
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                  <table class="datatable table table-hover table-striped table-bordered tc-table">
                    <tbody>
		                      ...
		                      <label>Country</label>
		                      <select class="form-control maincat required" name="cid" id="cid" >
		                        <option value="">Select Country</option>
		                        <?php $sth = $db->query ("SELECT * FROM `countries` ORDER BY `name` ASC");
		                              while($row = $sth->fetch()) {
		                    ...
		                    <div class="form-group col-sm-6">
		                      <label>State</label>
		                      <select class="form-control subcat showexp required" name="sid" >
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `cities` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php }
		...
		$guid = $_GET['guid'];
		$sth = $db->query ("DELETE FROM `cities` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'cities.php');
	} else {
	   if(!empty($post_msg)) { ?>
        <div class="page-header no-padding-bottom"> <?php echo $post_msg; ?></div><div class="clearfix"></div>
        <?php  }
        $sth = $db->query ("SELECT `cu`.`name`, `st`.`name`, `ci`.`guid`, `ci`.`name` AS `city` FROM `cities` AS `ci` LEFT JOIN `countries` AS `cu` ON `cu`.`guid` = `ci`.`cid` LEFT JOIN `provience` AS `st` ON `st`.`guid` = `ci`.`sid` ORDER BY `ci`.`guid` DESC");
		$count = $sth->rowCount();
	?>
    <a href="<?php echo URL; ?>cities.php?action=addNew" class="btn btn-info btn-xs">Add New </a>
    <div class="row">
      <div class="col-lg-12">
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  <thead>
                    <tr>
                      <th data-class="expand" class="col-medium center">S.No</th>
											<th data-class="expand" class="col-medium center">Country</th>
											<th data-class="expand" class="col-medium center">State/Provience</th>
                      <th data-class="expand" class="col-medium center">City Name</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><?php echo $row[0]; ?></td>
											<td class="col-medium center"><?php echo $row[1]; ?></td>
											<td class="col-medium center"><?php echo $row['city']; ?></td>
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>cities.php?action=editData&guid=<?php echo $row['guid']; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>cities.php?action=deleteData&guid=<?php echo $row['guid']; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
...
<script type="text/javascript">
    ...
    document.getElementById('rowfield').value=dd;
		...
		$("#customFields").append('<tr><td>City Name</td><td align="right"><div align="right">:</div></td><td><div align="center"><input type="text" name="name[]" required  class="form-control"></div></td><td><input value="0" name="chkRemoveAuthor[0]" type="checkbox"   class="form-control"/></td></tr>');
