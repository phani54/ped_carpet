...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	foreach($name as $names){
	 $sth = $db->exec ("INSERT INTO `provience`(`name`,`sid`) VALUES ('$names','$sid')");
	}
	if($sth > 0) {
	 $post_msg = '<h4 style="color: green;">Provience added successfully </h4>';
	 header('location:'.URL.'provience.php?sid='.$sid.'&post_msg='.$post_msg);
	} else {
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `provience` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
 ...
 $tth = $db->prepare ("SELECT * FROM `countries` WHERE `guid` = '$sid'");
 $tth-> execute();
 $trow = $tth-> fetch();
?>
<!DOCTYPE html>
...
<div id="wrapper">
  ...
  <div id="page-wrapper">
      ...
      <div class="col-lg-12">
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li> <a href="countries.php"><?php echo $trow[1]; ?></a></li>
            <li class="active"><a href="<?php echo URL; ?>provience.php?sid=<?php echo $sid; ?>">State/Provience</a></li>
          </ul>
          <div class="b-right hidden-xs">
            <ul>
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>provience.php?action=addNew&sid=<?php echo $sid; ?>">Add New</a></li>
                  <li><a href="<?php echo URL; ?>provience.php?sid=<?php echo $sid; ?>">View State/Provience</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title">
          <!-- PAGE TITLE ROW -->
          <h1><?php echo $trow[1]; ?> <span class="sub-title"><a href="<?php echo URL; ?>provience.php?sid=<?php echo $sid; ?>">State/Provience</a></span></h1>
        </div>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `provience` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php }
		...
		$sth = $db->query ("DELETE FROM `provience` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'provience.php?sid='.$sid);
	} else { if(!empty($post_msg)) { ?>
     <div class="page-header no-padding-bottom"><?php echo $post_msg; ?></div><div class="clearfix"></div>
    <?php }?>
    <a href="<?php echo URL; ?>provience.php?action=addNew&sid=<?php echo $sid; ?>" class="btn btn-info btn-xs">Add New</a>
    <div class="row">
      <div class="col-lg-8">
        <?php
		$sth = $db->query ("SELECT * FROM `provience` WHERE sid='$sid'");
		$count = $sth->rowCount();
		?>
        ...
        <div class="portlet table-responsive"><!-- /Portlet -->
          <div class="portlet-heading dark">
            <div class="portlet-title">
              <h4><?php echo $trow[1]; ?> - State/Provience</h4>
            </div>
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  <thead>
                    <tr>
                      ...
                      <th data-class="expand" class="col-medium center">Name</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>provience.php?action=editData&guid=<?php echo $row[0]; ?>&sid=<?php echo $row[1]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>provience.php?action=deleteData&guid=<?php echo $row[0]; ?>&sid=<?php echo $row[1]; ?>" class="btn btn-danger"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
