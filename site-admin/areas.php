...
if(isset($_POST) && $_POST['Submit'] == 'add') {
	...
	{
	$sth = $db->exec ("INSERT INTO `areas`(`name`,`sid`) VALUES ('$names','$sid')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Areas added successfully </h4>';	
	header('location:'.URL.'areas.php?sid='.$sid.'&post_msg='.$post_msg);
	} else { 
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$sth = $db->exec ("UPDATE `areas` SET `name`='$name' WHERE `guid`=$guid");
	
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Area Successfully Updated</h4>';	
	header('location:'.URL.'areas.php?sid='.$sid.'&post_msg='.$post_msg);
	} else { 
	$post_msg = '<h4 style="color: red;">Please try Again</h4>';	
	header('location:'.URL.'areas.php?sid='.$sid.'&post_msg='.$post_msg);
 ...
 $tth = $db->query ("SELECT * FROM `cities` WHERE `guid` = '$sid'");
 $trow = $tth-> fetch();
?>
<!DOCTYPE html>
<html lang="en">
...
<div id="wrapper">
  ...
  <div id="page-wrapper"> 
      ...
      <div class="col-lg-12"> 
        ...
        <div class="breadcrumbs">
          <ul class="breadcrumb">
            ...
            <li> <a href="cities.php"><?php echo $trow[1]; ?></a></li>
            <li class="active"><a href="<?php echo URL; ?>areas.php?sid=<?php echo $sid; ?>">Areas</a></li>
          </ul>
            ...
            <ul>
              ...
              <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                  <li><a href="<?php echo URL; ?>areas.php?action=addNew&sid=<?php echo $sid; ?>">Add New</a></li>
                  <li><a href="<?php echo URL; ?>areas.php?sid=<?php echo $sid; ?>">View Areas</a></li>
                </ul>
              </li>
            </ul>
          </div>
        ...
        <div class="page-header title"> 
          <!-- PAGE TITLE ROW -->
          <h1><?php echo $trow[1]; ?> <span class="sub-title"><a href="<?php echo URL; ?>areas.php?sid=<?php echo $sid; ?>">Areas</a></span></h1>
        </div>
        ...
        <div class="portlet">
          ...
          <div id="f-6" class="panel-collapse collapse in">
            <div class="portlet-body">
              <?php $sth = $db->prepare ("SELECT * FROM `areas` WHERE `guid` = '$guid'");
			      $sth-> execute();
				  $row = $sth-> fetch();
				?>
              <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <?php } 
		...
		$sth = $db->query ("DELETE FROM `areas` WHERE `guid`='$guid'");
		//$sth->execute();
		header('location:'.URL.'areas.php?sid='.$sid); 
	} else { 
	   if(!empty($post_msg)) { ?>
        <div class="page-header no-padding-bottom"> <?php echo $post_msg; ?> </div><div class="clearfix"></div>
        <?php }
			 $sth = $db->query ("SELECT * FROM `areas` WHERE sid='$sid'");
			 $count = $sth->rowCount();
	?>
    <a href="<?php echo URL; ?>areas.php?action=addNew&sid=<?php echo $sid; ?>" class="btn btn-info btn-xs">Add New </a>
    <div class="row">
      <div class="col-lg-8">
                ...
                <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                  <thead>
                    <tr>
                      ...
                      <th data-class="expand" class="col-medium center">Area Name</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>areas.php?action=editData&guid=<?php echo $row[0]; ?>&sid=<?php echo $row[1]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>areas.php?action=deleteData&guid=<?php echo $row[0]; ?>&sid=<?php echo $row[1]; ?>" class="btn btn-danger"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
                    <?php $m++; } } ?>
                  </tbody>
                </table>
