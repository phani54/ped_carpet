...
$sth = $db->query ("SELECT * FROM `countries` ORDER BY `guid` ASC");
$count = $sth->rowCount();
...
if(isset($_POST) && $_POST['Submit'] == 'add') {
   ...
   // if($count < 10) {
	foreach($name as $names){
	 $sth = $db->query ("INSERT INTO `countries`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Countries Successfully Created</h4>';
	header('location:'.URL.'countries.php?post_msg='.$post_msg);
	} else {
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$name = stripslashes($_POST['Name']);
 	$sth = $db->exec ("UPDATE `countries` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
  ...
  <div id="main-container">
        ...
        <div class="col-lg-12">
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>countries.php">Countries</a></li>
            </ul>
              ...
              <ul>
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php //if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>countries.php?action=addNew">Add New</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>countries.php">View Countries</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title">
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="countries.php">Countries</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
	...
	$guid = $_GET['guid'];  ?>
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php
										$sth = $db->query ("SELECT * FROM `countries` WHERE `guid` = '$guid'");
				  					$row = $sth-> fetch();
								?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      ...
      <?php }
				...
				$guid = $_GET['guid'];
				$sth = $db->query ("DELETE FROM `countries` WHERE `guid`='$guid'");
				//$sth = $db->query ("DELETE FROM `sub_category` WHERE `sid`='$guid'");
				header('location:'.URL.'countries.php');
			} else { ?>
          <?php if(!empty($post_msg)) { ?>
          <div class="page-header no-padding-bottom"> <?php echo $post_msg; ?> </div><div class="clearfix"></div>
          <?php  } ?>

      <a href="<?php echo URL; ?>countries.php?action=addNew" class="btn btn-xs btn-info">Add New</a>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                    <thead>
                      <tr>
                        ...
                        <th data-class="expand" class="col-medium center">Country Name</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="col-medium center"><?php echo $row[1]; ?></td>
												<td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>provience.php?sid=<?php echo $row[0]; ?>"  class="btn btn-inverse" title="View"><i class="fa fa-link icon-only"></i></a></div></td>
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>countries.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>countries.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
