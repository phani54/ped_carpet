...
$sth = $db->query ("SELECT * FROM `languages` ORDER BY `guid` ASC");
$count = $sth->rowCount();
...
if(isset($_POST) && $_POST['Submit'] == 'add') {
   ...
   // if($count < 10) {
	foreach($name as $names){
	 $sth = $db->query ("INSERT INTO `languages`(`name`) VALUES ('$names')");
	}
	if($sth > 0) {
	$post_msg = '<h4 style="color: green;">Languages Successfully Created</h4>';
	header('location:'.URL.'languages.php?post_msg='.$post_msg);
	} else {
...
if(isset($_POST) && $_POST['submit'] == 'SaveEdit') {
	...
	$name = stripslashes($_POST['Name']);
 	$sth = $db->exec ("UPDATE `languages` SET `name`='$name' WHERE `guid`=$guid");
	if($sth > 0) {
  ...
  <div id="main-container">
        ...
        <div class="col-lg-12">
          ...
          <div class="breadcrumbs">
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="<?php echo URL; ?>languages.php">Languages</a></li>
            </ul>
              ...
              <ul>
                <li class="dropdown"><a href="#" title="" data-toggle="dropdown"><i class="fa fa-plus"></i><span> Tasks</span></a>
                  <ul class="dropdown-menu dropdown-primary dropdown-menu-right">
                    <?php //if($count < 10) {?>
                    <li><a href="<?php echo URL; ?>languages.php?action=addNew">Add New</a></li>
                    <?php //} ?>
                    <li><a href="<?php echo URL; ?>languages.php">View Languages</a></li>
                  </ul>
                </li>
              </ul>
            </div>
          ...
          <div class="page-header title">
            <!-- PAGE TITLE ROW -->
            <h1>Admin <span class="sub-title"><a href="languages.php">Languages</a></span></h1>
          </div>
          <!-- /#ek-layout-button -->
	...
	$guid = $_GET['guid'];  ?>
          ...
          <div class="portlet">
            ...
            <div id="f-6" class="panel-collapse collapse in">
              <div class="portlet-body">
                <?php
										$sth = $db->query ("SELECT * FROM `languages` WHERE `guid` = '$guid'");
				  					$row = $sth-> fetch();
								?>
                <form role="form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      ...
      <?php }
				...
				$guid = $_GET['guid'];
				$sth = $db->query ("DELETE FROM `languages` WHERE `guid`='$guid'");
				header('location:'.URL.'languages.php');
			} else { ?>
          <?php if(!empty($post_msg)) { ?>
          <div class="page-header no-padding-bottom"> <?php echo $post_msg; ?> </div><div class="clearfix"></div>
          <?php  } ?>

      <a href="<?php echo URL; ?>languages.php?action=addNew" class="btn btn-xs btn-info">Add New</a>
      <div class="row">
        <div class="col-lg-12">
                  ...
                  <table id="SampleDT" class="datatable table table-hover table-striped table-bordered tc-table ">
                    <thead>
                      <tr>
                        ...
                        <th data-class="expand" class="col-medium center">Language</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="col-medium center"><div class="btn-group btn-group-sm"> <a href="<?php echo URL; ?>languages.php?action=editData&guid=<?php echo $row[0]; ?>" class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> <a href="<?php echo URL; ?>languages.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a></div></td>
                      </tr>
                      <?php $m++; } } ?>
                    </tbody>
                  </table>
